...
- Local identities are opt-in and available only through the Node adapter,
  including the container. Enabling `LOCAL_IDENTITIES_BACKEND=file` requires
  Node.js 24.7.0 or newer, one process writing a persistent private directory,
  an explicit, non-rotating `SUBJECT_SALT`, `LOCAL_IDENTITY_DOMAINS`, and a
  `STATE_STORE_BACKEND`. See [local identities](docs/local-identities.md).
- Relying parties accepting local sign-in must handle an absent
  `email_verified` claim. A local password, even with MFA, does not verify
  control of the address. Local subjects derive from a stable record id;
  explicitly linking an upstream to a local identity makes both sign-in
  routes use that local subject, which differs from an existing email-derived
  subject.
- Local TOTP seeds are plaintext Base32 values in private credential files.
  Protect the directory and encrypt backups. Records from the unreleased
  sealed-TOTP draft need offline conversion or replacement seeds before use;
  see the [migration guidance](docs/local-identities.md#totp-and-backup-codes).
- Deployment pipelines can now verify the complete published release and pin
  its authenticated container digest. Follow the
  [release verification instructions](docs/release-verification.md); pulling
  an image tag alone does not verify its signature or provenance.

### Added

- Operator-provisioned, file-backed local identities with Argon2id passwords,
  TOTP authenticators, single-use backup codes, and bounded profile claims.
  The `generate-local-identity` command provisions records and rekeys retained
  upstream refresh credentials. There is no self-service registration or
  credential-management API.
- Local sign-in routing by configured email domain, with eligible upstreams
  offered as alternatives. Password and second-factor attempt limits fail
  closed through the state store; unknown and unusable records receive the
  same form and dummy password-verification work.
- Stable local subjects and explicit upstream links matched by upstream id,
  verified issuer, exact subject, and canonical email address. Linked upstream
  refresh tokens can be retained as sealed credentials, but are not used to
  extend sessions or issued to relying parties.
- Authoritative local-record checks when reusing sessions and redeeming
  authorisation codes. Disabling or deleting an account, or advancing its
  security version, invalidates those sessions and unredeemed codes.
- `urn:sag:acr:local-password` and `urn:sag:acr:local-mfa` authentication
  contexts, plus the method-neutral `urn:sag:acr:mfa` requirement. The latter
  accepts local or federated MFA while tokens retain the actual method's
  `acr` and `amr`; specifically requested local and federated contexts remain
  separate.
- Password-manager hints, a numeric authenticator-code input, and a separate
  backup-code form for local sign-in. Complete authenticator codes submit
  after a short pause; both forms also work without JavaScript.
...
- CI and release gates now include security-oriented static analysis and
  coverage-guided fuzzing. Core line coverage is gated at 100%, with a checked
  coverage badge generated by the test suite.
- Email-verification evidence is carried explicitly through sessions,
  authorisation codes, and access tokens. Existing upstream and email-code
  sessions and grants retain their verified-address status during upgrades.

### Fixed

- The email sign-in field now uses a valid username autocomplete token so
  browsers and password managers can identify it correctly.
- DNS provider hints reject domain labels longer than 63 characters and
  labels beginning or ending with a hyphen.
